// Probe that mode/material/alpha are three independent axes:
// changing any one must produce an observable visual change. At
// α=1 the three modes used to converge to identical OPAQUE-routed
// renders; this probe captures the same alpha across modes and
// flags any pair that produces byte-identical screenshots.
...
const OUT = join(__dirname, '_out', 'wboit_independence');
...
async function main() {
    ...
    // For each (material, alpha), capture all three modes and compare.
    // Modes should produce DIFFERENT hashes within a single (mat, α)
    // group — if any two modes hash-equal at the same α, that means
    // the transition is a no-op visually.
    ...
    for (const mat of MATERIALS) {
        ...
        for (const alpha of ALPHAS) {
            ...
            results[`${mat}_a${alpha.toFixed(2)}`] = group;

            // Detect collisions: any pair of modes producing the same hash
    ...
    // Cross-check: alpha independence at fixed (material, mode)
    ...
    for (const mat of MATERIALS) {
        ...
        for (const mode of MODES) {
            ...
            process.stdout.write(`  ${tag} ${mat}/${mode}: alpha values produced ${distinct}/${group.length} distinct hashes\n`);
        }
    }
